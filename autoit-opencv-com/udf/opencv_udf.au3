...
Func _OpenCV_get($vVal = Default)
	...
	$OPENCV_SAMPLES_DATA_PATH = _OpenCV_FindFile("samples\data", Default, Default, Default, _OpenCV_Tuple( _
			"opencv\sources", _
			"opencv-4.13.0-*\sources", _
			"opencv-4.13.0-*\opencv\sources" _
			))
	If FileExists($OPENCV_SAMPLES_DATA_PATH) Then
		$cv.samples.addSamplesDataSearchPath($OPENCV_SAMPLES_DATA_PATH)
	EndIf
...
Func _OpenCV_Install($s_opencv_world_dll = Default, $s_autoit_opencv_com_dll = Default, $bUser = Default, $bOpen = True, $bClose = True, $bInstall = False, $bUninstall = False)
	If $s_opencv_world_dll == Default Then $s_opencv_world_dll = "opencv_world4130.dll"
	If $s_autoit_opencv_com_dll == Default Then $s_autoit_opencv_com_dll = "autoit_opencv_com4130.dll"
	...
	; ffmpeg is looked on PATH when loaded in debug mode, not relatively to opencv_world4130d.dll
	; this is a work around to load ffmpeg relatively to opencv_world4130d.dll
	If $bClose And $h_opencv_ffmpeg_dll <> -1 Then DllClose($h_opencv_ffmpeg_dll)
	If $bOpen And EnvGet("OPENCV_BUILD_TYPE") == "Debug" Then
		$h_opencv_ffmpeg_dll = _OpenCV_LoadDLL(StringReplace($s_opencv_world_dll, "opencv_world4130d.dll", "opencv_videoio_ffmpeg4130_64.dll"))
