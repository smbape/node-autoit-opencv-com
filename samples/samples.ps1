...
$sDllCom = "$PSScriptRoot\..\autoit-opencv-com\build_x64\bin\$($BuildType)\autoit_opencv_com4130$($PostSuffix).dll"
$bFound = Test-Path -Path $sDllCom
if (!$bFound) {
    $sDllCom = "$PSScriptRoot\..\autoit-opencv-com\autoit_opencv_com4130$($PostSuffix).dll"
}

[AutoItOpenCV]::DllOpen(
    "$PSScriptRoot\..\opencv-4.13.0-windows\opencv\build\x64\vc16\bin\opencv_world4130$($PostSuffix).dll",
    $sDllCom
)
...
$cv.samples.addSamplesDataSearchPath("$PSScriptRoot\..\opencv-4.13.0-windows\opencv\sources\samples\data")
